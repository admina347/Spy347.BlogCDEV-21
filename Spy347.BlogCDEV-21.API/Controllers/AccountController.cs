...
namespace Spy347.BlogCDEV_21.API.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        [HttpGet]
        [Route("GetUsers")]
        public async Task<IEnumerable<UserEditViewModel>> GetUsers()
        {
            var users = _accountService.GetAccounts().Result;
            var userResponse = users.Select(u => new UserEditViewModel { Id = new Guid(u.Id), Email = u.Email, UserName = u.UserName, Posts = u.Posts.Select(_ => _.Id)}).ToList();
            return userResponse;
        }
        ...
        [HttpPost]
        [Route("AddUser")]
        public async Task<IActionResult> AddUser(RegisterViewModel request)
        {
